...
class Compiler:
    ...
    def compile(self):
        ...
        out = self.pass_1(out)
        offsets = self.do_variables(len(out)+1)
    ...
    def add_variable(self,name,type,val,delta=1):
        if not name in self.variables:
            self.variables[name] = [var(type,"var"+str(self.var_start),self.var_start,val)]
            self.var_start += delta
            return
        self.variables[name].append(var(type,"var"+str(self.var_start),self.var_start,val))
        self.var_start += delta
    ...
    def do_variables(self,offset):
        out = []
        for i in self.variables.keys():
            for j in self.variables[i]:
                temp = "#" + str(j.address) + " " + str(j.addr_offset + offset)
                out.append(temp)
        return out
        ...
        for line in inp:
            ...
            if len(command) > 1:
                ...
                if command[0] == "_load":
                    ...
                    if file_to_load[0] == "&":
                        file_to_load = file_to_load[1:]
                        out.append("CPY MEMORY_DIR " + str(self.variables[file_to_load][0].address) + " REGISTER_DIR GA")
                    else:
